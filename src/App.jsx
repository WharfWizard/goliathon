...
async function downloadPdf(sectionKey,dossier){
  ...
  const sections={
    ...
    institutionresponse:{title:"What They Will Likely Argue",data:()=>[{type:"body",text:(dossier.institution_response?"Based on the evidence filed so far:\n\n"+dossier.institution_response:"Not yet generated.")+"\n\n"+AI_DISCLAIMER_PDF_TEXT}]},
    ...
    complete:{title:"Complete Evidence Dossier",data:()=>{
      ...
      if(dossier.decision_summary){items.push({type:"sectionHeader",title:"Decision-Maker Summary"});items.push({type:"body",text:dossier.decision_summary+"\n\n"+AI_DISCLAIMER_PDF_TEXT});}if(dossier.institution_response){items.push({type:"sectionHeader",title:"What They Will Likely Argue"});items.push({type:"body",text:"Based on the evidence filed so far:\n\n"+dossier.institution_response+"\n\n"+AI_DISCLAIMER_PDF_TEXT});}
...
export default function GoliathonApp(){
  ...
  const handleCameraSubmit=useCallback(async()=>{
    ...
    try{
      ...
      const prompt=`${existing}\n\nAnalyse this photographed document (${cameraPages.length} pages) and return ONLY valid JSON:\n{"case_title":"short title","evidence_item":{"title":"title","date":"DD Mon YYYY or null","type":"Letter/Court Document/Medical/Financial/Other","summary":"2-3 sentence factual summary of what this document shows","facts_observed":"one sentence listing only what is directly stated or shown in the document — no interpretation","significance":"one sentence explaining why this matters to the case — clearly interpretive"},"timeline_entry":{"date":"DD Mon YYYY or null","event":"one sentence","evidence":"reference"},"overview_update":"updated overview","witness_update":"new sentences only","next_steps_update":"numbered 3-5 actions. Where any action involves escalation, include one plain sentence about potential costs exposure.","key_questions_update":"updated list of 3-5 plain-language questions this case still needs to answer","institution_response":"one plain sentence: the strongest argument the institution is likely to make against this case based on the evidence, or state that the evidence does not yet support a prediction.","decision_summary_update":"updated one-page decision-maker summary with the same structure as the main prompt: what the case is about, core dispute, strongest evidence, institution's likely argument, power imbalance if relevant, what happens next."}`;
        ...
        {(processing||threatProcessing)?(
          ...
          <div>
            ...
            <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}}>
              <label onClick={e=>e.stopPropagation()} style={{cursor:"pointer",background:YELLOW,color:NAVY,borderRadius:8,fontFamily:"'Poppins',sans-serif",fontWeight:700,fontSize:12,padding:"7px 16px",display:"inline-flex",alignItems:"center",gap:6,whiteSpace:"nowrap",letterSpacing:0.5}}>📎 Upload File<input type="file" accept="image/*,.pdf,.txt,.html,.htm,.doc,.docx,.msg" style={{display:"none"}} onChange={e=>{if(e.target.files[0])handleFile(e.target.files[0]);}}/></label>
...
<AIDisclaimer/>
            ...
            {dossier.institution_response&&<Panel title="What They Will Likely Argue" icon="⚔️"><p style={{margin:"0 0 8px",fontSize:12,color:"#a0b4c8",lineHeight:1.6}}>Based on the evidence filed so far. Check this against the actual response when it arrives — if it differs, use the correction loop in your evidence items.</p><p style={{margin:0,fontSize:13,lineHeight:1.8,color:LIGHT,whiteSpace:"pre-wrap"}}>{dossier.institution_response}</p><AIDisclaimer/></Panel>}
